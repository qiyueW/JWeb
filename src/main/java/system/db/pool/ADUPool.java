...
final public class ADUPool {
	...
	public void denyAutoClose() {
		...
		for (Map.Entry<Integer, ADUSession> eobj : map.entrySet()) {
			if (eobj.getValue().isUnLock()) {
				eobj.getValue().Lock();
				try {
					statement = eobj.getValue().getConn().createStatement();
					statement.executeQuery("SELECT 1");
					System.out.println("ADUPool:denyAutoClose doing....");
					statement.close();
					eobj.getValue().close();
		...
		for (Map.Entry<Integer, ADUSession> entry : map.entrySet()) {
			...
			try {
				if (null != cc && !cc.isClosed()) {
					cc.close();
					sb.append("\nADU:成功关闭一个连接").append(cc).append("//the index: ").append(entry.getKey());
